"""TFLite schema vocabulary: builtin operator codes and tensor types.

Values mirror the public append-only enums in tensorflow/lite/schema/schema.fbs.
Names are the exact TFLite builtin operator strings (vocabulary rule: no
aliases, no prettified names). Codes not in this table render as
``UNKNOWN_BUILTIN_<code>`` and classify as ``unknown``.
"""
...
BUILTIN_NAME_TO_CODE: dict[str, int] = {name: code for code, name in BUILTIN_OPERATORS.items()}

TENSOR_TYPES: dict[int, str] = {
    0: "float32",
    1: "float16",
    2: "int32",
    3: "uint8",
...
DTYPE_TO_TENSOR_TYPE: dict[str, int] = {name: code for code, name in TENSOR_TYPES.items()}


def builtin_name(code: int) -> str:
    return BUILTIN_OPERATORS.get(code, f"UNKNOWN_BUILTIN_{code}")


def tensor_type_name(code: int) -> str:
    return TENSOR_TYPES.get(code, f"unknown_dtype_{code}")
