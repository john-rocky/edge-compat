...
| **Command** | `python convert_lfm25.py LiquidAI/LFM2.5-1.2B-JP out_lfm25_12b_fp --fp && python ../minicpm_work/quantize_litertlm.py apply out_lfm25_12b_fp/model.litertlm lfm25_int8.litertlm --recipe wi8fc` |
...
|---|---|---|
| `LFM2.5-1.2B-JP_int8.litertlm` | `8fc5367c35a4771cb60784a86be5eb1f358e94cc71eb2eaf15ad006205339001` | 1186.938 |
...
Model-level on-device results — measured behavior of this exact artifact on the recorded device, only meaningful together with that environment. Throughput is conditional on the measured prompt length (Prompt (tokens); '-' = the source stated none): rows differing only there are different measurements, not re-runs. Full records (failure classes, log evidence): `data/device_runs/0.16.0/2026-08-12/lfm2.5-1.2b-jp-int8__mac-studio-m4-max.json`, `data/device_runs/0.16.0/2026-08-24/lfm2.5-1.2b-jp-int8__galaxy-s26.json`, `data/device_runs/0.16.0/2026-09-05/lfm2.5-1.2b-jp-int8__galaxy-s26.json`, `data/device_runs/0.16.1/2026-09-01/lfm2.5-1.2b-jp-int8__raspberry-pi-5.json`.

| Device | Accelerator | Status | Full delegation | Output match | Prompt (tokens) | Latency p50 (ms) | Prefill tok/s | Decode tok/s | TTFT (ms) | Peak mem (MB) | Environment | Date | Provenance |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| galaxy-s26 | cpu | fallback | no | - | 205 | - | 485.05 | 41.32 | 450.0 | - | Galaxy S26 (SM-S942Q) · Qualcomm SM8850 · litert-lm 0.16.0 · Android 16 | 2026-09-05 | measured |
...
| raspberry-pi-5 | cpu | pass | - | - | 256 | - | 74.86 | 7.03 | 3562.2 | 1888.0 | Raspberry Pi 5 Model B Rev 1.1 · Broadcom BCM2712 · litert-lm 0.16.1 · Linux-6.18.34+rpt-rpi-2712-aarch64-with-glibc2.41 | 2026-09-01 | measured |

## Pitfalls

- Do not reuse the Instruct sibling's export-time int8 command here. Export-time conv-int8 costs this Japanese tune 9 GSM8K points (56 against 65 for the linears-only recipe), which is exactly why the published JP int8 file is post-hoc wi8fc while the published Instruct int8 file is the export-time recipe. Conv-int8 sensitivity is per-finetune and must be A/B'd, not assumed.
...
- GSM8K (greedy, 0-shot CoT, n=100) is 65 for this int8 file against a bf16 reference of 63.
